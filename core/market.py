...
class SingleInstrumentMarket(Market):
    ...
    def execute_order(self,timestamp: pd.Timestamp, order: Order) -> Optional[Trade]:
        current_market_price = self.get_price(timestamp)

        # TODO: when getting to fractional orders, need to rewrite this
        if order.quantity == 0:
            logger.info(f"Not Executed: Zero order size found for {order}")
            return None
